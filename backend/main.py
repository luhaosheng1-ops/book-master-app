...
- **强制使用 Mermaid 输出"第一性原理推导链"**，格式如下（必须包含节点颜色样式）：
  ```mermaid
  graph TD
      A[公理A] --> B[推论B]
      B --> C[定理C]
      ...
      style D fill:#95e1d3,stroke:#20c997,stroke-width:2px
  ```
- 展示从最底层公理到最终结论的完整推导路径
- 标注每个节点的充要条件和依赖关系
- 用数学符号表达：A → B → C 的逻辑链
- **必须使用 style 代码为关键节点设置颜色**：公理节点用红色，推论节点用青色，定理节点用黄色，应用节点用绿色
...
- **强制输出"100天行动计划"表格**，将书中的核心方法转化为100天的具体行动计划
- 按周划分，每周有明确的目标和任务
- 每天有具体的执行步骤和检查点
- **必须使用 Markdown 表格格式**：
  | 周次 | 阶段目标 | 每日任务 | 检查点 | 预期成果 |
  |------|---------|---------|-------|---------|
  | Week 1-4 | [目标] | [任务1, 任务2, ...] | [检查标准] | [成果指标] |
  | Week 5-8 | [目标] | [任务1, 任务2, ...] | [检查标准] | [成果指标] |
  | ... | ... | ... | ... | ... |
- 覆盖完整的100天，每周至少列出3-5个具体任务

二、决策风险矩阵 (Decision Risk Matrix)
- **强制输出"决策风险矩阵"表格**，列出所有关键决策点
- 对每个决策，评估：
  - 风险等级（高/中/低）
  - 收益预期（高/中/低）
  - 执行难度（高/中/低）
  ...
  - 建议（执行/暂缓/放弃）
- **必须使用 Markdown 表格格式**：
  | 决策点 | 风险等级 | 收益预期 | 执行难度 | 时间成本 | 建议 |
  |--------|---------|---------|---------|---------|------|
  | 决策X | 高/中/低 | 高/中/低 | 高/中/低 | [时间] | 执行/暂缓/放弃 |
  | 决策Y | 高/中/低 | 高/中/低 | 高/中/低 | [时间] | 执行/暂缓/放弃 |
- 至少列出5-10个关键决策点
...
三、认知边界 (Cognitive Boundaries)
- **强制包含"认知边界"板块**：分析该书理论在何种极端环境下会失效
- 明确标注理论适用的边界条件
- 列出理论失效的具体场景和案例（至少3-5个）
- 说明为什么在这些边界外理论不成立
- 提供识别边界的方法
- **必须使用表格或列表格式**：
  | 理论/观点 | 适用条件 | 失效边界 | 失效原因 | 识别方法 |
  |----------|---------|---------|---------|---------|
  | 理论X | [条件A] | [边界B] | [原因C] | [方法D] |
  | 理论Y | [条件A] | [边界B] | [原因C] | [方法D] |
...
请开始脱水："""
        ...
        async def generate_stream():
            ...
            try:
                ...
                # 使用 asyncio.gather 并发执行所有任务（3.5升级：改为gather）
                yield f"data: {json.dumps({'type': 'status', 'msg': f'开始并发解构 {total_chunks} 个片段（使用 asyncio.gather 加速）...'}, ensure_ascii=False)}\n\n"
                yield f"data: {json.dumps({'type': 'progress', 'val': 35}, ensure_ascii=False)}\n\n"
                ...
                dehydrated_chunks = [result[1] for result in results]
                
                # 发送完成进度
                yield f"data: {json.dumps({'type': 'status', 'msg': f'所有 {total_chunks} 个片段并发解构完成！'}, ensure_ascii=False)}\n\n"
